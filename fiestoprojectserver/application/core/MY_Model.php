...
class MY_Model extends CI_Model {
    ...
    function get_column_max_length($column_name) {
        $model_data = $this->get_model_data();
        $result = my_get_row_result(get_query_column_max_length($model_data['table_name'], $column_name));
        if ($result['DATA_TYPE'] == "varchar" ||
            ...
            $column_type = explode("(", str_replace(")", "", $result['COLUMN_TYPE']));
            if ($result['DATA_TYPE'] == "decimal") {
                $decimal_length = explode(",", $column_type[1]);
                $column_type_max_length = $decimal_length[0];
            } else {
                $column_type_max_length = $column_type[1];
                if ($result['DATA_TYPE'] == "int" ||
                        $result['DATA_TYPE'] == "tinyint" ||
                        $result['DATA_TYPE'] == "bigint") {
                    $column_type_max_length = $column_type_max_length + num_of_integer_separator($column_type_max_length);
                }
            }
            $column_max_length = $column_type_max_length;
        } else if ($result['DATA_TYPE'] == "date") {
            $column_max_length = 10;
        } else if ($result['DATA_TYPE'] == "datetime") {
            $column_max_length = 19;
        } else {
            $column_max_length = 0;
        }
    ...
    function get_all_by_field($field_value, $offset = -1, $field = "", $search = "") {
        $model_data = $this->get_model_data();
        if ($field == "") {
    ...
    function get_all_by_module_field($field_value, $offset = -1, $field = "", $search = "") {
        $model_data = $this->get_model_data();
        return $this->get_all_by_field($field_value, $offset, $model_data['table_name'] . "." . $field, $search);
    }
    ...
    function get_all($offset = -1, $search = "") {
        $model_data = $this->get_model_data();
        return my_get_all($model_data['query'] . " " . $this->module_search($search) . " " . $this->order_by(), $offset);
    }
    ...
    function get_single_result($sql_query) {
    ...
    function get_num_rows($search = "") {
        ...
        if (isset($model_data['get_num_rows_query']) && $model_data['get_num_rows_query'] != "") {
            ...
            $get_num_rows_query = build_get_num_rows_query($model_data['query']);
        }
